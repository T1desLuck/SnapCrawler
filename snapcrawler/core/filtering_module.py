...
class FilteringModule:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                try:
                    timeout = self.config.get('crawling', {}).get('timeouts', {}).get('queue_timeout', 30)
                ...
                if item.get('type') == 'image_url':
                    ...
                    if self.compact_formatter:
                        self.compact_formatter.update_stats(
                            ...
                            images_failed=self.filtered_count,
                            images_downloaded=self.downloaded_count,
                            images_saved=self.processed_count,
    ...
    def download_image(self, url: str) -> str:
        ...
        try:
            ...
            # Успешная загрузка — сбрасываем признак ошибки для компактной строки
            if self.compact_formatter:
            ...
            with open(raw_path, 'wb') as f:
                chunk_size = self.config.get('crawling', {}).get('timeouts', {}).get('chunk_size', 8192)
            ...
            if isinstance(e, requests.HTTPError) and getattr(e, 'response', None) is not None:
                ...
                try:
                    ...
                    pass
            # Обновляем компактную статистику кодом ошибки HTTP, если доступно
            if self.compact_formatter and status:
                try:
                    self.compact_formatter.update_stats(has_errors=True, error_code=int(status))
                except Exception:
                    self.compact_formatter.update_stats(has_errors=True, error_code=status)
            ...
            if count <= 3 or count % 50 == 0:
                if self.verbose_logging:
                    ...
                    # В компактном режиме без подробностей
                    self.logger.debug(f"Скачивание не удалось ({status})")
            else:
                self.logger.debug(f"Скачивание не удалось ({key}) x{count}")
            return None
    ...
    def apply_filters(self, image_path: str) -> bool:
        ...
        try:
            ...
            # Дедупликация по перцептивному хэшу
            if self.filtering_config.get('deduplication', True):
                phash = imagehash.phash(img)
                if phash in self.image_hashes:
                    if self.verbose_logging:
                        self.logger.info(f"❌ [{filename}] Дубликат по perceptual hash")
                    return False
                self.image_hashes.add(phash)
            
            # Проверка на водяные знаки
            watermark_config = self.filtering_config.get('watermark_detection', {})
            if watermark_config.get('enabled', False):
                if self.has_watermark(image_path):
                    if self.verbose_logging:
                        self.logger.info(f"❌ [{filename}] Обнаружен водяной знак")
                    return False
            
            # Все фильтры пройдены
            if self.verbose_logging:
                color_mode = img.mode
                self.logger.info(f"✅ [{filename}] Прошел все фильтры: {img.size}, {color_mode}")
            return True
    ...
    def has_watermark(self, image_path: str) -> bool:
        ...
        try:
            ...
            sensitivity = watermark_config.get('sensitivity', 50)
